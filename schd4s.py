...
# ******************************************************************************
@module
def modSchedule4s(input, output, session):
    # Radiologists -------------------------------------------------------------
    @reactive.calc
    def radiologistsDf():
    ...
    def excelWorkbook() -> Path:
        ...
        xlTitle = 'B2'
        xlDates: list[str] = ['F5:L5', 'F21:L21', 'F29:L29']
        weekLabel = f'{weekStart:%Y%b%d}-{weekEnd:%d}'

        wb = openpyxl.load_workbook(Path(__file__).parent / 'FourShiftWeeklyRoster.xlsx')
        ws  = wb['template']
        ws.title = weekLabel
    ...
    def rosterUI():
        ...
        def RadiologistSelector():
            ...
            with ui.layout_column_wrap(width=1/8):
                ...
                # Shift D
                ui.tags.strong('D (0000-0600)')
                ui.input_selectize('slxShiftD0', label=None, choices=rads, remove_button=True, multiple=False)
                ui.input_selectize('slxShiftD1', label=None, choices=rads, remove_button=True, multiple=False)
                ui.input_selectize('slxShiftD2', label=None, choices=rads, remove_button=True, multiple=False)
                ui.input_selectize('slxShiftD3', label=None, choices=rads, remove_button=True, multiple=False)
                ui.input_selectize('slxShiftD4', label=None, choices=rads, remove_button=True, multiple=False)

                ui.input_selectize('slxShiftD5', label=None, choices=rads, remove_button=True, multiple=False)
                ui.input_selectize('slxShiftD6', label=None, choices=rads, remove_button=True, multiple=False)

                # Shift A
                ui.tags.strong('A (0600-1200)')
    ...
    def clearSelectors():
        selectors = [
            'slxShiftD0', 'slxShiftD1', 'slxShiftD2', 'slxShiftD3', 'slxShiftD4', 'slxShiftD5', 'slxShiftD6',
    ...
    def rosterDf():
        ...
        data = [
            ['D', input.slxShiftD0(), input.slxShiftD1(), input.slxShiftD2(), input.slxShiftD3(), input.slxShiftD4(), input.slxShiftD5(), input.slxShiftD6()],
    ...
    def updateRoster():
        ...
        xlSectionRanges: dict[Sections, list[str]] = {
           Sections.XR: ['F30:L30', 'F31:L31', 'F32:L32', 'F33:L33'],
           Sections.US: ['F22:L22', 'F23:L23', 'F24:L24', 'F25:L25'],
           Sections.NEURO: ['F6:L6', 'F9:L9', 'F12:L12', 'F15:L15'],
           Sections.NON_NEURO_PLAIN: ['F7:L7', 'F10:L10', 'F13:L13', 'F16:L16'],
           Sections.NON_NEURO_CONTRAST: ['F8:L8', 'F11:L11', 'F14:L14', 'F17:L17'],
        }
